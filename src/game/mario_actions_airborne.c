...
void play_far_fall_sound(struct MarioState *m) {
    ...
    if (!(action & ACT_FLAG_INVULNERABLE) && action != ACT_TWIRLING && action != ACT_FLYING
        ...
        if (m->peakHeight - m->pos[1] > 4150.0f) {
...
s32 act_jump_kick(struct MarioState *m) {
    ...
    if (m->actionState == 0) {
        play_sound_if_no_flag(m, SOUND_ACTION_TWIRL, MARIO_ACTION_SOUND_PLAYED);
...
s32 mario_execute_airborne_action(struct MarioState *m) {
    ...
    switch (m->action) {
        ...
        case ACT_TRIPLE_JUMP:          cancel = act_twirling(m); set_mario_action(m, ACT_TWIRLING, 0); set_mario_action(m, ACT_TWIRLING, 0); set_mario_action(m, ACT_TWIRLING, 0); set_mario_action(m, ACT_TWIRLING, 0); play_mario_sound(m, SOUND_MARIO_YAHOO, 1);          break;
